 ...
 */
template <typename T> //this creates a Type T which is not yet specified, sucht that the function T can take any argument data type
static inline T gold_red(const T* const ary, const size_t N) //inline: function code is copied at calling location treduce overhead
{
    T sum = 0.0;
    for (size_t i = 0; i < N; ++i)
        sum += ary[i];
